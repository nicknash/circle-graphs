...
{
    void CombinedOutputSensitive::updateAt(std::stack<int> &pendingUpdates, std::vector<int> &MIS, int indexToUpdate, int newMisValue)
    {
        MIS[indexToUpdate] = newMisValue;
        pendingUpdates.push(indexToUpdate);
    }

    bool CombinedOutputSensitive::tryUpdate(const cg::data_structures::DistinctIntervalRep &intervals, std::stack<int> &pendingUpdates, IndependentSet& independentSet, const cg::data_structures::Interval &newInterval, std::vector<int> &MIS, std::vector<int> &CMIS, int maxAllowedMIS)
    {
    ...
    std::optional<std::vector<cg::data_structures::Interval>> CombinedOutputSensitive::tryComputeMIS(const cg::data_structures::DistinctIntervalRep &intervals, int maxAllowedMIS)
    {
        ...
        for (auto i = 0; i < intervals.end; ++i)
        {
            ...
            if (maybeInterval)
            {
                ...
                if (!tryUpdate(intervals, pendingUpdates, independentSet, interval, MIS, CMIS, maxAllowedMIS)) // ! OK LET'S SEE IF WE CAN GET O(n min d alpha) cleanly
                {
                    return std::nullopt;
                }
            }
